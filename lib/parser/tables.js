      ...
      case $.TP_PHP:
        return this.newNodeAndNext('PHPBlockStatement', {
          value: this.token.text
            .replace(/^\s*{\s*php\s*}([^]+){\/\s*php\s*}\s*$/m, '$1')
        });
...
//          modifiers: [],
  ...
  tag: function() {
    ...
    switch(this.token.id) {
      ...
      case $.TP_LDELFOREACH:
        ...
        res.attributes = (res.attributes || []).filter(function (v) {
          if (v.key.name === 'from' || v.key.name === 'key' || v.key.name === 'item' || v.key.name === 'name') {
            res[v.key.name] = v.value;
            return false;
          }
          return true;
        });
        ...
        switch (this.token.id) {
          ...
          case $.TP_DOLLAR: // varindexed started with TP_DOLLARID or TP_DOLLAR
            ...
            if (this.token.id === $.TP_DOLLARID && this.token.next.id === $.TP_EQUAL) {
              left = this.newNode('Identifier', { name: this.token.text });
              this.next().next();
            } else {
  ...
  attribute: function() {
    ...
    if (this.token.id === $.TP_ATTR) {
      var key = this.newNodeAndNext('Identifier', {
        name: this.token.text.replace(/^\s*|=$/g, '')
      });
    ...
    while (postfix =
      ...
      if (Array.isArray(postfix)) {
        res = this.newNode('BinaryExpression', {
          operator: normalizeOperator(postfix[0].text),
          raw: postfix[0].text,
          left: res,
          right: postfix[1],
        });
      } else if (postfix[0].id === $.TP_SINGLECOND) { // single condition
        // this.trace('SCOND UNARY', postfix);
  ...
  value: function() {
    ...
    if (res = this.try([$.TP_OPENP, $_.expr, $.TP_CLOSEP])) {
      return res[1];
    }
    ...
    if (!res && (res = this.try($_.variable))) {
      if (this.token.id === $.TP_INCDEC) {
        res = this.newNodeAndNext('UpdateExpression', {
          operator: normalizeOperator(this.token.text),
          raw: this.token.text,
          argument: res,
          prefix: false,
        });
    ...
    switch (res? 0 : this.token.id) {
      ...
      case $.TP_ID:
        ...
        break;
      case $.TP_NAMESPACE:
        res = this.newNodeAndNext('Identifier', { name: this.token.text });
        break;
      case $.TP_HEX:
    ...
    if (prefix) {
      res = this.newNode('UnaryExpression', {
        operator: normalizeOperator(prefix.text),
        raw: prefix.text,
        argument: res,
        prefix: true,
      });
      // this.trace('UNIMATCH', res, prefix);
    }
   ...
   */
  variable: function() {
    if (this.token.id === $.TP_DOLLARID && [$.TP_DOT, $.TP_OPENB, $.TP_PTR, $.TP_AT].indexOf(this.token.next.id) === -1) {
      return this.newNodeAndNext('Identifier', { name: this.token.text });
    }
    ...
    if (body = this.try([$_.varvar, $.TP_AT, $.TP_ID])) {
      return this.newNode('MemberExpression', {
        object: body[0],
        property: this.newNode('Identifier', { name: body[2].text, raw: body[2].text }),
        computed: false,
        pointer: false,
        at: true,
      });
    ...
    if (this.is($.TP_DOT)) {
      ...
      switch (this.token.id) {
        ...
        case $.TP_INTEGER:
          ...
          break;
        case $.TP_DOLLARID:
          property = this.newNodeAndNext('Identifier', { name: this.token.text });
          break;
        case $.TP_LDEL:
  ...
  varvar: function() {
    ...
    var ref = this.is($.TP_DOLLARID)?
      this.newNode('Identifier', { name: this.token.text })
  ...
  object: function() {
    ...
    var object = this.try($_.varindexed)
      || this.newNodeAndNext('Identifier', { name: this.token.text });
  ...
  doublequoted_with_quotes: function() {
    ...
    // this.trace('DQUOTED', body);

    if (body.length <= 1 && (!body[0] || body[0].type !== 'Identifier')) {
      return this.newNode('Literal', { value: body[0] || '', doublequoted: true });
    }
