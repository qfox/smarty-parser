...
var $_ = module.exports = {
  ...
  literal: function() {
    ...
    while (this.token.id !== $.EOF && this.token.id !== $.TP_LITERALEND) {
      if (this.token.id === $.TP_LITERALEND) this.throwError('Unclosed {literal} block', {});
  ...
  smartytag: function() {
    switch (this.token.id) {
      case $.TP_SIMPLETAG:    return this.newNode('Tag', { id: this.token.text });
      // case $.TP_SIMPLEOUTPUT: return this.newNode('SIMPLEOUTPUT', { id: this.token.text });
      case $.TP_CLOSETAG:
        // this.trace('SMARTYTAG CLOSETAG');
        return this.newNodeAndNext('ClosingStatement', {
...
//          modifiers: [],
  ...
  tag: function() {
    ...
    switch(this.token.id) {
      ...
      case $.TP_LDELIF:
        ...
        var attributes = this.try($_.attributes);
        return { test: test, attributes: attributes };

      case $.TP_LDELFOR:       return this.next().read('for');
      case $.TP_LDELFOREACH:   return this.next().read('forEach');
      ...
      case $.TP_LDELSLASH:
        ...
        return this.newNode('ClosingStatement', {
          id: id,
      ...
      case $.TP_LDEL:
        ...
        switch (this.token.id) {
          ...
          case $.TP_ID:
            var node = this.newNodeAndNext('Statement', { id: this.token.text });
  ...
  expr: function() {
    ...
    while (postfix =
      ...
      if (Array.isArray(postfix) && postfix[0].id) {
        switch (postfix[0].id) {
          case $.TP_MATH:
          case $.TP_UNIMATH:
          case $.TP_ISIN:
      ...
      } else if (false) { // single condition
