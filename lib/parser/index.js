  ...
  _readElementsUntil: function(lastToken) {
    ...
    while (this.token.id !== tokens.ids.EOF && this.token.id !== lastToken) {
      ...
      for (var i = res.length - 1; i >= 0; i--) {
        ...
        if (elem.id === 'if') {
          var consequent = [], alternate = [], elseFound = false;
          for (var j = 0, l = body.length; j < l; j++) {
            if (body[j].type === 'TagStatement' && body[j].id === 'else') {
              elseFound = true;
              continue;
            }
            (elseFound? alternate : consequent).push(body[j]);
          }
          node.consequent = consequent.length? consequent : null;
          node.alternate = alternate.length? alternate : null;
        } else {
          node.body = body;
        }
