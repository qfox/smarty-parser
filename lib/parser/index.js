...
Parser.prototype = Object.create({
  ...
  throwError: function(data) {
    ...
    var loc = data.loc?
      ...
      : _normalizeLocation(this.token.loc);
    var msg = 'Unexpected "' + token + '"' + msgExpect + ' at ' + String(loc.start) + ': ';
    ...
    throw new SyntaxError({
      message: msg,
      loc: loc,
      parent: this._tryErrors? this._tryErrors[this._tryErrors.length - 1] : null
    });
  },
  ...
  _readElementsUntil: function(lastToken) {
    ...
    while (this.token.id !== tokens.ids.EOF && this.token.id !== lastToken) {
      ...
      for (var i = res.length - 1; i >= 0; i--) {
        var node = res[i];
        if (!(node.type === 'TagStatement' && elem.id === node.id
          || node.type === 'IfStatement' && elem.id === 'if'
          || node.type === 'WhileStatement' && elem.id === 'while'
          || node.type === 'ForEachStatement' && elem.id === 'foreach')) {
          continue;
        }
        if (node._filled) {
          continue;
        ...
        // console.log(node.type, node.id, elem.type, elem.id);
        node.type = node.type === 'TagStatement'? 'TagBlockStatement' : node.type;
        var body = res.splice(i + 1);

        var consequent = [],
          alternate = [],
          elseFound = false;
        if (elem.id === 'foreach') {
          for (var j = 0, l = body.length; j < l; j++) {
            if (body[j].type === 'TagStatement' && body[j].id === 'foreachelse') {
              elseFound = true;
              continue;
            }
            (elseFound? alternate : consequent).push(body[j]);
          }
          node.body = consequent;
          node.alternate = elseFound? alternate : null;
          debugger;
          Object.defineProperty(node, '_filled', { value: true });
          // this.trace('ReadElementsUtil', node);
        } else if (elem.id === 'if') {
          var subnode = node;
          for (var j = 0, l = body.length; j < l; j++) {
            if (body[j].type === '_ElseIfStatement') {
              subnode.consequent = consequent.length? consequent : null;
              subnode.alternate = [ body[j] ];
              Object.defineProperty(subnode, '_filled', { value: true });
              body[j].type = 'IfStatement';
              subnode = body[j];
              consequent = [];
              continue;
            } else if (body[j].type === 'TagStatement' && body[j].id === 'else') {
              elseFound = true;
              continue;
            }
            (elseFound? alternate : consequent).push(body[j]);
          ...
          subnode.consequent = consequent.length? consequent : null;
          subnode.alternate = alternate.length? alternate : null;
          Object.defineProperty(subnode, '_filled', { value: true });
        } else {
          node.body = body;
          Object.defineProperty(node, '_filled', { value: true });
